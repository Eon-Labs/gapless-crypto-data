...
class TestCLIIntegration:
    ...
    def test_legacy_cli_btcusdt_single_day(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            metadata_file = Path(temp_dir) / "binance_spot_BTCUSDT-1h_20240101-20240101_v2.5.0.metadata.json"
    ...
    def test_subcommand_cli_ethusdt_single_day(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([
                "uv", "run", "gapless-crypto-data", "collect",
                "--symbol", "ETHUSDT",
                "--timeframes", "1h",
                "--start", "2024-01-01",
                "--end", "2024-01-01",
            ...
            metadata_file = Path(temp_dir) / "binance_spot_ETHUSDT-1h_20240101-20240101_v2.5.0.metadata.json"
    ...
    def test_cli_version_integration(self):
        ...
        result = subprocess.run([
            "uv", "run", "gapless-crypto-data", "--version"
        ], capture_output=True, text=True)
    ...
    def test_metadata_completeness(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(metadata_file, 'r') as f:
                ...
                required_fields = [
                    'version', 'generator', 'generation_timestamp', 'data_source',
                    'symbol', 'timeframe', 'actual_bars', 'date_range',
                    'statistics', 'data_integrity', 'gap_analysis', 'compliance'
                ]
                ...
                assert metadata['compliance']['zero_magic_numbers'] == True
                assert metadata['compliance']['temporal_integrity'] == True